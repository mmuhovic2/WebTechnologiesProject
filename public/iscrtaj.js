...
function iscrtajRaspored(div, dani, satPocetak, satKraj) {
    if (satPocetak >= satKraj || satPocetak < 0 || satPocetak > 24 || satKraj < 0 || satKraj > 24 ||
        Number.isInteger(satPocetak) == false || Number.isInteger(satKraj) == false || dani.length == 0) {
...
function vratiStringSata(sat) {
    let string = "";
    sat < 10 ? (string = "0" + sat.toString() + ":00") : (string = sat.toString() + ":00");
    return string;
}

function vratiVrijeme(tabela) {
...
function provjeriPoklapanje(red, satPocetak, vrijemePocetak, vrijemeKraj) {
    let pocetak = 2 * (vrijemePocetak - satPocetak);
    let trajanje = 2 * (vrijemeKraj - vrijemePocetak);
    let brojac = 0;
    for (let i = 1; i < red.cells.length; i++) {
        if ((brojac < pocetak && brojac + red.cells[i].colSpan > pocetak) ||
            (brojac >= pocetak && brojac < pocetak + trajanje && red.cells[i].innerHTML != "")) return true;
...
function provjeriDan(tabela, dan) {
